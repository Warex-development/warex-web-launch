...
const BENEFITS = [
  ...
  { icon: Star, title: 'Quality Control', color: 'text-blue-400 bg-blue-400/10', desc: 'Every listing is manually reviewed by WareXhub\'s quality team. OEM numbers verified against manufacturer databases before any listing goes LIVE.' },
  ...
  { icon: Users, title: 'Admin-Backed', color: 'text-red-400 bg-red-400/10', desc: 'A dedicated WareXhub operations team manages every deal. Manual matching ensures the best fit. Not an algorithm — real expertise behind every match.' },
...
export default function WhyWareXhubPage() {
  return (
    <div className="min-h-screen pt-20 pb-24">
      <div className="page-container">
        <div className="text-center py-16">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-[#1A1A1A] mb-4 leading-tight">Why Choose <span className="gradient-text">WareXhub?</span></h1>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="glass-card p-8 mb-24">
          <h2 className="text-2xl font-bold text-[#1A1A1A] mb-8 text-center">WareXhub vs. Traditional Sourcing</h2>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#E5E5E5]">
                  <th className="text-left py-3 px-4 text-[#333333] font-medium">Feature</th>
                  <th className="text-center py-3 px-4 text-[#4A3A5C] font-bold">WareXhub</th>
        ...
        <div className="text-center">
          <Link to="/register" className="btn-primary px-10 py-4 text-base inline-flex">
            Get Started With WareXhub <ArrowRight className="w-5 h-5" />
          </Link>
        </div>
      </div>
    </div>
